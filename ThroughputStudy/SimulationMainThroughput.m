function [tputProg] = SimulationMainThroughput(parallelInfo,runInfo,specInfo,fitsname, ...
    optical,starInfo,source,SpecOrImager,conditions, headerinfo,persistentSource)
% Creates all simulation objects, combines throughput with flux,simlulates
% physical instrument response (PSF or spectrum) and writes data product
% and records parameter lists
%
% Inputs
% ------
% processingInfo :
...
for ii = tracenum
    ...
    if any(strcmp('demonstrator', source{ii}.throughput)) == 1 && exist('demonstrator','var') == 0
        % make the a flat instrument throughput
        demo = Imager('demonstrator');
        imagerComponents = [imagerComponents, demo];
    end
...
for ii = tracenum
    ...
    if strcmp(SpecOrImager,'Imager')
        ...
        [frame] = Simulation.simulateImager(psf/4,dimensions,pixelpitch,counts/FR); % currently uses gaussian beam but should use FT code for PSF generation.
    ...
    if any(strcmp('spectrograph', source{ii}.throughput)) == 1
        ...
        if (source{ii}.tput_flag) == 0 % if thoughput is off
            
            for jj = 1:size(spectrograph.finalThroughput{1},2) % make a perfect grating to get orders
        ...
        %calculate average final throughput 
        ...
        for ii = 1:size(wave_coeff,1)
            ...
            det_eff(:,ii) = interp1(tputProg{1,end}{1,1}(:,ii),tputProg{1,end}{1,2}(:,ii),det_wave*1000);
        end
        
        simulation.intTrans = mean(mean(det_eff));
        
        clear specCell tempTput new_y det_eff det_wave 
...
% At this point you have a spectral_cell of
% {trace}{order}(wavelength,counts).% Now convolve each spectral order and
% clip on detector in parallel or serial.
...
headerinfo2 = {
    'Imag', star.imag,' ';...
    'Rmag', star.rmag,' ';...
    'rho',mean(smfInjection.finalThroughput(:,2)),' ';...
               };
headerinfo = [headerinfo;headerinfo2];
simulation.WriteFits(fitsname,frame,headerinfo)
fprintf('%s \n',char(hex2dec('2713')))
end
