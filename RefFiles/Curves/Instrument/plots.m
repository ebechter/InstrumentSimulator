...
% Figure 1
% makePlot('ICOS_',saving)

% Figure 2 
makePlot('ICOS_ADC',saving)

% Figure 3 
% makePlot('RMI_',saving)
% 
% % Figure 4 
% makePlot('RMIe',saving)

% % Figure 5
% makePlot('ProtectedGold45',saving)
% 
% % Figure 6
% makePlot('SilicaAR',saving)
...
for i = 1:length(files)
    a = load(files{i},'-mat');
    fields = fieldnames(a);
    h{i}=plotOneCurve(a.(fields{1})(:,1),a.(fields{1})(:,2),color(i,:),2,'Wavelength (nm)','Efficiency');
...
if saveflag
    tightfig(gcf)
    savefig(gcf,['customARcoatings/' prefix])
%     fig=gcf;
