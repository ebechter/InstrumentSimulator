...
%                 disp(ii)
        ...
        function [y_new_resamp] = Resamp_Extrap(new_x,new_y,x)
            ...
            extrap_scalar = min(new_y);
            ub = min(new_x(end,1),x(end,1));
            ind = find(new_x<=ub,1,'last');
            x_safe = new_x(1:ind);
            y_safe = new_y(1:ind);
            
            y_new_resamp = interp1(x_safe(:,1),y_safe(:,1),x,'pchip',extrap_scalar);
