classdef FiberCouplingV2
    properties
        ...
        %Calculated tolerances/ parameters
        BeamPos
        FiberPos
        F
        gridparams
    ...
    methods
        function [obj] = FiberCouplingV2(wavelength,wfe,adc,fiberpos,dof,dispersion)
            % Pre Initialization. Any code not using output argument (obj)
            
            % The coupling efficiency, rho, is given in terms of the overlap integral of
            ...
            % Source1 : Coupling of a semiconductor laser to a single-mode fiber,
            % Author : Nu Yu, February 1987
            % Source2: A Study of degraded light coupling into SMFs
            % Author C. Ruilier
        ...
        function [obj] = PrepareFiberOffsets(obj)
            ...
            if obj.dispersion == true
                ...
                Offset = y1.*0; % remove residual dispersion.
                
                obj.BeamPos = Offset; % lateral offset of the fiber translates to phase in pupil plan
        ...
        function [obj] = CreatePupilPlane(obj)
            ...
            for z = 1:length(obj.WFE)
                ...
                [maskr,maskc] = size(Z);
                Z = padarray(Z,[375,375]);% increase padding on output;
            ...
            circpix2 = 1-circpix2;
            circpix = logical(circpix.*circpix2);
            obj.rmsWFE = rms(W(circpix));
            obj.rmsPhase = rms(angle(obj.Pupil(circpix)));
            ...
            for ii = 1:length(obj.Wavelength)
                ...
                %test = opd2psf(pupil,W,obj.Wavelength*1e-6);
        ...
        function [obj] = CreateFiberPSF(obj)
            ...
            for ii = 1:size(obj.Wavelength,1)
                ...
                x = parGridx(:,:,ii);
                y = parGridy(:,:,ii);
                FiberPSF = exp(-(((x-mu(ii,1)).^2+(y-mu(ii,2)).^2)/(2*(sigma)^2)));
                FiberPSF = sqrt(FiberPSF);
...
%Calculate the MFD
function [MFD]=SMF_MFD(lambda)
% units must be in microns!!
...
% function G = ft2(g,delta)
% G = fftshift(fft2((g)))*delta^2;
% end
% %Inverse 2D Fourier transform
% function g = ift2(G,delta_f)
% N = size(G,1);
% g = ifft2(ifftshift((G)))*(N*delta_f)^2;
% end
...
Uout = 1/(1i*wvl*f).*exp(1i*k/(2*f)*(1-d/f)*(x2.^2 +y2.^2))...
    .*ft2(Uin,dl);
end

%ZERNIKE CALC - Started use 7-25-17
