    ...
    methods
        ...
        function [obj] = loadOpticalModelCurves(obj,curveDirectory)
            ...
            for ii =1:n %loop over cellarry size (i.e. surfaces)
                ...
                if isfield(obj.opticalModel{ii},'internalTrans')
                    internalTrans = obj.opticalModel{ii}.internalTrans;   
                end
        ...
        function [] = progressPlot(obj)
            ...
            science = viridis(size(obj.opticalModel,2)+1);
...
%             clear d
            ...
            hold on
            for ii = 1:length(obj.progress)
                h{ii} = plot(obj.progress{ii}(:,1),obj.progress{ii}(:,2),'-','LineWidth',1.5,'Color',colors{ii});
                handle = [handle h{ii}];
            end
    ...
    methods(Static)
        ...
        function[x,y]= multiply_curves(x1,y1,x2,y2)
            ...
            samp = min(max(diff(x1)),max(diff(x2)));
            
            y1 = interp1(x1,y1,(x1(1):samp:x1(end))','pchip','extrap'); 
            x1 = (x1(1):samp:x1(end))';
            
            y2 = interp1(x2,y2,(x2(1):samp:x2(end))','pchip','extrap');
            x2 = (x2(1):samp:x2(end))';
            ...
            x = x(out1:length(x)); % partial bounded x  
            
            Int = Int(out1:length(Int)); %partial bounded x
            ...
            [out1,~] = find(wav2(end)>= x ,1,'last');
            
            x = x(1:out1); %x vector bounded
            
            Int = Int(1:out1); % y1 vector bounded
            
            Int2 = Int2(1:out1); %y2 vector bounded          
